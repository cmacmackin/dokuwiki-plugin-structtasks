...
END,
    ...
    const edit_message = 'Some text summarising changes made.';
    ...
    public function setUp() : void {
        parent::setUp();
        $url = DOKU_BASE . DOKU_SCRIPT . '?id=' . $this::page_id;
...
END;
    ...
    private function makeMockMailer($calls = 1) {
    ...
    public function provideNotifiers() {
        $assigned_mock_mailer = $this->makeMockMailer();
        $assigned_mock_mailer->expects($this->once())
                             ->method('to')
                             ->with($this->equalTo($this::new_data['assignees'][0]));
        return [
    ...
    public function testNotifiers($notifier, $mailer) {
        $n = new $notifier('self::fakeGetConf', self::getLangCallback('assigned'));
        $n->sendMessage(
            $this::page_id,
            $this::page_title,
            $this::editor,
